...
class FeatureExtractor:
    ...
    def extract_security_features(self, events: pd.DataFrame) -> List[List[float]]:
        ...
        # Updated required_columns to match the standardized names from CICIDS2017Processor
        # FIXED: Changed 'flowduration' to 'flow_duration' to match standardized column names.
        required_columns = [
            'total_fwd_packets', 'total_backward_packets', 'total_length_of_fwd_packets',
            'total_length_of_bwd_packets', 'flow_duration', 'flow_bytes_s', 'flow_packets_s',
        ...
        for _, row in events.iterrows():
            feature_row = [
                ...
                row['flow_duration'], # Changed from flow_duration
